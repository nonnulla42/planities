...
import { MousePointer2, Plus, Check, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Move as MoveIcon, Hand, FileText, Box, RotateCcw, RotateCw, Crosshair, Printer } from 'lucide-react';
...
export const ManualTracer: React.FC<ManualTracerProps> = ({
  ...
  const getRelativePos = useCallback((e: React.MouseEvent | MouseEvent): Point => {
    ...
    const y = canvasHeight - (e.clientY - rect.top - offset.y) / zoom;
    return { x, y };
  ...
  const findPointAt = useCallback((pos: Point): { wallIndex: number, type: 'start' | 'end' } | null => {
    const threshold = getWorldDistanceForPixels(POINT_HIT_RADIUS_PX);
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#E4E3E0] select-none">
      {/* Overlays */}
      <AnimatePresence>
        {isCalibrating && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 20 }}
            className="absolute bottom-24 left-1/2 -translate-x-1/2 z-50 bg-white p-6 rounded-3xl shadow-2xl border border-[#141414]/10 min-w-[320px]"
      ...
      <div 
        className="flex-1 relative overflow-hidden bg-[#D1D0CD]" 
        ref={containerRef}
      ...
      >
        <div className="absolute left-6 top-6 z-30 w-[168px] space-y-3" ref={leftToolbarRef} onMouseDown={stopUiMouseDown}>
          <div className="relative flex items-stretch gap-2" ref={drawingGuideAnchorRef}>
            <button
          ...
          <div className="relative flex items-stretch gap-2" ref={interactionGuideAnchorRef}>
            <button
          ...
          <div className="relative flex items-stretch gap-2" ref={environmentGuideAnchorRef}>
            <button
        ...
        {/* Status Bar */}
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-6 py-2 bg-[#141414]/80 backdrop-blur-md text-white rounded-full z-20 flex items-center gap-4 shadow-xl border border-white/10">
          ...
          >
            ...
            >
              <g transform={`translate(0, ${mode === 'trace' ? TRACE_CANVAS_HEIGHT : SCALE_CANVAS_HEIGHT}) scale(1, -1)`}>
                ...
                {isMultiWallRotationMode && selectedWallsRotationPivot && (
                  <g pointerEvents="none">
                    <circle
